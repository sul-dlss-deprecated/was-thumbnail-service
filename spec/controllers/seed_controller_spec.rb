...
RSpec.describe Api::SeedController, :type => :controller do
  ...
  describe 'Get create' do
    it 'creates a new record if it does not exist in db' do
      ...
      post :create, params: {:druid=> 'ab123cd4567',:uri=>'http://test1.edu/'}
      expect(response).to have_http_status(200)
      expect(SeedUri.all.length).to eq(1)
    end
    it 'raises an error if there is no parameters or one is missing' do
      expect{post :create}.to raise_error(ActionController::ParameterMissing)
      expect{post :create, params: {:druid=> 'ab123cd4567'}}.to raise_error(ActionController::ParameterMissing)
      expect{post :create, params: {:uri=> 'http://example.org'}}.to raise_error(ActionController::ParameterMissing)
    end
