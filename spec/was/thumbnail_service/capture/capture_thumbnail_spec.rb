...
describe Was::ThumbnailService::Capture::CaptureThumbnail do
  ...
  describe '.capture' do
    before do
      FileUtils.rm("#{Settings.thumbnail_tmp_directory}/19990104000000.jpeg") if File.exist?("#{Settings.thumbnail_tmp_directory}/19990104000000.jpeg")
    end
    ...
    after :each do
      FileUtils.rm("#{Settings.thumbnail_tmp_directory}/19990104000000.jpeg") if File.exist?("#{Settings.thumbnail_tmp_directory}/19990104000000.jpeg")
      FileUtils.rm("#{Settings.thumbnail_tmp_directory}/19990104000000.jp2") if File.exist?("#{Settings.thumbnail_tmp_directory}/19990104000000.jp2")
    end
  end
  ...
  describe '.save_to_stack' do
    ...
    after :all do
      FileUtils.rm_r("#{@fixtures}/image_stacks/aa/") if File.exist?("#{@fixtures}/image_stacks/aa/")
    end
  end

  describe '.update_database' do
    before :all do
      @memento11 = Memento.create(id: 10_001, uri_id: 1001, memento_uri: 'https://swap.stanford.edu/19980901000000/http://test1.edu/', memento_datetime: '1998-09-01 00:00:00', is_selected: 'true', is_thumbnail_captured: 'false')
    end
    it 'updates is_thumbnail_captured for the memento record in the database' do
      capture_thumbnail = CaptureThumbnail.new(10_001, '', 'https://swap.stanford.edu/19980901000000/http://test1.edu/', '19980901000000')
      capture_thumbnail.update_database

      expect(Memento.find(10_001)[:is_thumbnail_captured]).to be true
    end
    it 'raises an error if the memento id is not found' do
      capture_thumbnail = CaptureThumbnail.new(10_002, '', '', '')
