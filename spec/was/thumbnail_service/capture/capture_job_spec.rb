...
describe Was::ThumbnailService::Capture::CaptureJob do
  ...
  describe '.perform' do
    it 'calls process thumbnail based on memento id' do
      capture_job = CaptureJob.new(101,'druid:ab123cd4567')
      ...
      expect(CaptureThumbnail).to receive(:new).with(101,'druid:ab123cd4567','https://swap.stanford.edu/19980901000000/http://test1.edu/','19980901000000').and_return(capture_thumb)
      expect_any_instance_of(CaptureThumbnail).to receive(:process_thumbnail)
    
      capture_job.perform
    end
    it 'raises an error if the record is not found' do
      capture_job = CaptureJob.new(123,'druid:ab123cd4567')
